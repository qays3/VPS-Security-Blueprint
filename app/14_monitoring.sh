...
cat > /usr/local/bin/security-status.sh <<'DASHEOF'
#!/bin/bash
...
echo -e "Primary Interface: enp0s3"
...
score=0
systemctl is-active --quiet suricata 2>/dev/null && score=$((score + 15))
systemctl is-active --quiet snort 2>/dev/null && score=$((score + 10))
systemctl is-active --quiet fail2ban 2>/dev/null && score=$((score + 15))
systemctl is-active --quiet nginx 2>/dev/null && score=$((score + 10))
systemctl is-active --quiet wazuh-manager 2>/dev/null && score=$((score + 10))
command -v ufw >/dev/null && ufw status 2>/dev/null | grep -q "Status: active" && score=$((score + 10))
[[ $EUID -eq 0 ]] && iptables -L 2>/dev/null | grep -q "DROP" && score=$((score + 10))
[ -f /etc/nginx/modsec/main.conf ] && score=$((score + 10))
nginx -t 2>/dev/null && grep -q "modsecurity on" /etc/nginx/sites-enabled/* 2>/dev/null && score=$((score + 5))
grep -q "AllowUsers" /etc/ssh/sshd_config 2>/dev/null && score=$((score + 5))

if [ $score -ge 95 ]; then
    echo -e "Security Score: ${GREEN}$score/100 - EXCELLENT${NC}"
elif [ $score -ge 85 ]; then
    echo -e "Security Score: ${GREEN}$score/100 - VERY GOOD${NC}"
...
cat > /etc/logrotate.d/security-logs <<'LOGROTEOF'
